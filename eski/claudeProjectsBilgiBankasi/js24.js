        ...
        // ==================== 3D SCENE ====================
        // Ã¢Å“â€¦ FIX #1 & #2: Video listener'larÃ„Â± dÃƒÂ¼zgÃƒÂ¼n yÃƒÂ¶netim
    ...
    const handleLoadedMetadata = () => {
        debugLog('ÄŸÅ¸â€œÂ¹ Video metadata loaded, duration:', videoElement.duration);
    };
    ...
    if (isP2PRoom && !isRoomOwner) {
        // Ã„Â°zleyici: P2P'den indir
        try {
            showP2PStatus('ÄŸÅ¸â€Â P2P baÃ„Å¸lantÃ„Â±sÃ„Â± kuruluyor...', 0);
            const videoFile = await joinP2PTorrent(currentRoomData.p2p.magnetURI);
            
            // WebTorrent file'Ã„Â± video element'e baÃ„Å¸la
            videoFile.renderTo(videoElement, { autoplay: false }, (err) => {
                if (err) {
                    console.error('Render error:', err);
                    updateP2PStatus('Ã¢ÂÅ’ Video yÃƒÂ¼klenemedi', 0);
                } else {
                    debugLog('Ã¢Å“â€¦ P2P video rendered to element');
                }
            });
        } catch (e) {
            console.error('P2P join error:', e);
            updateP2PStatus('Ã¢ÂÅ’ P2P hatasÃ„Â±: ' + e.message, 0);
        }
    } else if (isP2PRoom && isRoomOwner) {
        // Sahip: Zaten seed ediyoruz, lokal dosyayÃ„Â± kullan
        if (selectedLocalFile) {
            const objectURL = URL.createObjectURL(selectedLocalFile);
            currentVideoObjectURL = objectURL;
            videoElement.src = objectURL;
            showP2PStatus('ÄŸÅ¸â€œÂ¤ PaylaÃ…Å¸Ã„Â±lÃ„Â±yor...', 100);
        }
    } else {
        // Normal URL modu
    ...
    if (isRoomOwner) {
        ...
        const buttons = [
            { text: 'Ã¢â€“Â¶', position: '-0.6 0 0', event: 'play' },
            { text: 'Ã¢ÂÂ¸', position: '-0.2 0 0', event: 'pause' },
            { text: 'Ã¢ÂÂª', position: '0.2 0 0', event: 'rewind' },
            { text: 'Ã¢ÂÂ©', position: '0.6 0 0', event: 'forward' }
        ];

        // Ã¢Å“â€¦ FIX: VR button listener'larÃ„Â±nÃ„Â± track et (cleanup iÃƒÂ§in)
