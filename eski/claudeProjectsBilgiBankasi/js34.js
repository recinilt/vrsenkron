        ...
        // Ã¢Å“â€¦ FIX #5 & #6: syncVideo - main thread blokajÃ„Â± ve DOM thrashing azaltma
        function syncVideo() {
            ...
            if (state.isPlaying) {
                ...
                // Ã¢Å“â€¦ FIX: elapsed NaN/Infinity validation (max 24 saat)
                if (!isFinite(elapsed) || elapsed < 0 || elapsed > 86400) {
                    debugLog('Ã¢Å¡Â Ã¯Â¸Â Invalid elapsed time, skipping sync');
                    return;
                }
            ...
            if (!state.isPlaying) {
                ...
                // Ã¢Å“â€¦ FIX #6: isBuffering flag'i pause'da temizle
                if (isBuffering) {
            ...
            if (drift <= TIER1_THRESHOLD) {
                ...
                // 3-9 saniye arasÃ„Â± drift - daha agresif playbackRate
                if (videoElement.paused) {
                    videoElement.play().catch(() => {});
                }
                const behind = currentTime < expectedTime;
                ...
                if (now - lastHardSeekTime < HARD_SEEK_MIN_INTERVAL || isHardSeeking) {
                    ...
                    const behind = currentTime < expectedTime;
                    videoElement.playbackRate = behind ? 1.75 : 0.65; // Ã¢Å“â€¦ FIX: Daha agresif
                    return;
                }
                ...
                const BUFFER_ADVANCE = 9; // Ã¢Å“â€¦ FIX: 7 -> 9 saniye
                ...
                const onHardSeeked = () => {
                    ...
                    isHardSeeking = false;
                    debugLog('Ã¢Å“â€¦ Hard seek completed (syncVideo)');
                };
                ...
                // Ã¢Å“â€¦ FIX: Timeout fallback - seeked event gelmezse temizle - TRACKED
                trackTimeout(setTimeout(() => {
                    if (isHardSeeking) {
                        videoElement.removeEventListener('seeked', onHardSeeked);
                        isHardSeeking = false;
                        debugLog('Ã¢Å¡Â Ã¯Â¸Â Hard seek timeout (syncVideo)');
                    }
                }, 3000));
                ...
                bufferCountdownInterval = setInterval(() => {
                    ...
                    // Ã¢Å“â€¦ countdownEl dÃƒÂ¶ngÃƒÂ¼ dÃ„Â±Ã…Å¸Ã„Â±nda cache'lendi
                    if (countdownEl) {
