...
// ODA SAHİBİ ODAKLI + İLK GİRİŞ SENKRONU
// ============================================

function listenToRoomUpdates() {
    if (!roomRef) return;
    
    // İLK GİRİŞTE HEMEN SENKRON BAŞLAT
    roomRef.child('videoState').once('value', (snapshot) => {
        ...
        // İlk giriş senkronizasyonu
        console.log('🎯 İlk giriş: Video durumu senkronize ediliyor...');
        
        const now = Date.now();
        
        // Pozisyonu ayarla
        videoElement.currentTime = state.currentTime;
        console.log(`   → Pozisyon: ${state.currentTime.toFixed(1)}s`);
        
        // Eğer video oynatılıyorsa
        if (state.isPlaying && state.startTimestamp) {
            const waitTime = state.startTimestamp - now;
            
            if (waitTime > 0) {
                // Henüz başlama zamanı gelmedi
                console.log(`   → ${(waitTime/1000).toFixed(1)}s sonra başlayacak`);
                videoElement.pause();
                ...
                syncTimeout = setTimeout(() => {
                    ...
                    videoElement.play().then(() => {
                        console.log('▶️ Video başlatıldı (İLK GİRİŞ SENKRON)');
                    }).catch(err => {
                        console.log('⚠️ Auto-play engellendi:', err);
                    });
                }, waitTime);
            } else {
                // Video zaten başlamış, geç kaldık
                ...
                videoElement.play().then(() => {
                    console.log(`▶️ Video başlatıldı (İLK GİRİŞ - ${elapsedSeconds.toFixed(1)}s gecikmeli)`);
                }).catch(err => {
                    console.log('⚠️ Auto-play engellendi:', err);
                });
            }
        } else {
            // Video duruyorsa sadece pozisyonu ayarla
            if (!videoElement.paused) {
                videoElement.pause();
            }
            console.log('   → Video durumda');
        }
    });
    
    // GERÇEK ZAMANLI DEĞİŞİKLİKLERİ DİNLE
    ...
    console.log('   → İlk giriş otomatik senkron');
...
function syncVideoState() {
    ...
    roomRef.child('videoState').once('value', (snapshot) => {
        ...
        if (state.isPlaying && videoElement.paused) {
            videoElement.play().catch(err => console.log('⚠️ Auto-play engellendi:', err));
