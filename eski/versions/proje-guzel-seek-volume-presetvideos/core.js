// ============================================
// CORE.JS - OPTİMİZE EDİLMİŞ VERSİYON V3
// Interval tracking + Cleanup iyileştirmeleri
// ============================================
...
async function syncServerTime() {
  ...
  for (let i = 0; i < 2; i++) { // GÜNCELLEME: 3 → 2 sample
    const t0 = Date.now();
    ...
    samples.push(offset);
    await new Promise(resolve => setTimeout(resolve, 200)); // GÜNCELLEME: 100ms → 200ms
  }
...
function startDriftTracking() {
  ...
  }, DRIFT_UPDATE_INTERVAL)); // Artık 5000ms
  
  if (DEBUG_MODE) console.log('📊 Drift tracking başlatıldı (5000ms interval)');
}
...
function seekVideo(seconds) {
  ...
  const tempDisable = getServerTime();
...
async function applySyncCorrection(targetTime, targetPlaying, urgentUpdate = false) {
  ...
  // GÜNCELLEME: 1000ms → 2000ms
  if (getServerTime() - lastSyncDisableTime < 2000) {
  ...
  // TIER CRITICAL: Otomatik senkronizasyon (15sn+)
  else {
    ...
    if (AUTO_RECONNECT_ENABLED && driftMs > AUTO_RECONNECT_THRESHOLD) {
      ...
      showSyncStatus('⚠️ Bağlantı sorunu! Lütfen yeniden katılın (15sn+ gecikme)');
    }
  }
...
if (DEBUG_MODE) console.log('✅ Core yüklendi - V3 Optimized Version');
