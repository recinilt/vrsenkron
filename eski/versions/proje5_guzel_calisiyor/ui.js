...
function joinRoom(roomId, password = null) {
    roomsRef.child(roomId).once('value').then((snapshot) => {
        ...
        auth.signInAnonymously().then(() => {
            ...
            console.log('✓ Odaya katıldı:', roomId, '| Sahip:', isRoomOwner);
        });
    });
}
...
function listRooms() {
    roomsRef.once('value').then((snapshot) => {
        ...
        if (!rooms) {
            list.innerHTML = '<p style="text-align:center;color:#999;">Oda yok</p>';
            return;
        }
        ...
        Object.entries(rooms).forEach(([rid, room]) => {
            ...
            div.innerHTML = `
                ...
                <div class="room-info">
                    👥 ${room.viewers || 0} | 
                    🎭 ${ENVIRONMENTS[room.environment]?.name || '?'} | 
                    🎮 ${room.controlMode === 'owner' ? 'Sahip' : 'Herkes'}
                </div>
            `;
        ...
        if (count === 0) {
            list.innerHTML = '<p style="text-align:center;color:#999;">Açık oda yok</p>';
        }
    });
}
...
function setupVideoTexture(videoUrl, screenSize) {
    ...
    videoElement.addEventListener('loadedmetadata', () => {
        console.log('✓ Metadata:', videoElement.duration.toFixed(1), 's');
    });
    
    videoElement.addEventListener('canplay', () => {
        console.log('✓ Oynatmaya hazır');
...
function loadSubtitle(url) {
    ...
    fetch(url).then(r => r.text()).then(text => {
        ...
        console.log('✓ Altyazı:', subtitleData.length, 'satır');
        startSubtitleUpdate();
    }).catch(err => {
        console.error('❌ Altyazı hatası:', err);
    });
}
